...
For single matching, a typical matcher should be able to match in $O(n)$ using this DFA, where $n$ is the length of the input string.
%However, this DFA does not allow for overlapped matching.
The matcher would need to be restarted after each starting position, leading to a running time complexity of $O(n^2)$.
