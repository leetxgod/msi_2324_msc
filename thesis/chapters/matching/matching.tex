...
\end{figure}
As seen above, there is no distinct branching of states. This means that we can't track the positions for each match, nor can we continually input text into the automaton.
Now, building using the modified position automaton construction from \ref{alg:nfaPosCount} results in the NFA represented below:
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[shorten >=1pt, node distance=2cm, on grid, auto, initial text=]
		\node[state, initial]            (q0) {0};
...
Consider the following regular expression $R$ and input word $w$:
\begin{center}
...
As seen above, we are able to get all the matches possible (even overlapped ones) for the given input word $w$, along with their positions.
