...
A word $w \in \Sigma^*$ is accepted by the NFA if there exists a sequence of transitions (possibly including $\varepsilon$-moves) that consumes $w$ and ends in a state $q$ such that $q \in F$.
Figure~\ref{fig:nfa_example} contains an example of a NFA.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[shorten >=1pt,node distance=2cm,on grid,auto,initial text=]
		...
		(q5) edge node {d} (q3);
	\end{tikzpicture}
	\caption{Example of an NFA that accepts $L = \{aba, a c^\star d\}$}
\label{fig:nfa_example}
\end{figure}



An NFA is \emph{deterministic} (also known as DFA) if $|\delta(q,\sigma)| \leq 1$, for any $(q,\sigma) \in Q \times \Sigma$ and $|Q_0| = 1$. Figure~\ref{fig:dfa_example} contains an example of a DFA.
...
%For a DFA, checking if $w \in L$ --- for a given word $w$ and a language $L$ over $\Sigma$ --- takes linear time with $|w|$.
...
%It is a powerful concept used to define the behavior of regular expressions in a more operational manner.
%Derivatives can be used to decide if a word belongs to $L(r)$ for a given regular expression $r$, or to decide if two regular expressions are equivalent or, even to obtain a DFA equivalent to the regular expression itself.
%They can be used as a means of verifying equivalence of regular expressions, for example.
...
Finally, the set of states for $\mathcal{D}$ is $Q = \{ (ad+d^*)db, ddb, d^*db + b, db, b, \varepsilon, \emptyset \}$ and the set of final states is $F = \{ \varepsilon \}$. The diagram for this automaton is shown in Figure~\ref{fig:brzozowski_derivatives_example}.
...
In order to parse expressions into FAdo's classes and types, \emph{lark} was used. \emph{lark} \cite{lark_parser} is a parsing toolkit for Python. It can parse any context-free language.
